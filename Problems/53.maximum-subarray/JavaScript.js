 ...
 */
// one 
var maxSubArrayON2 = function(nums) {
    var count = countArray(nums,0,nums.length-1);
    for (let i = 0; i < nums.length; i++) {
        for(let j = i; j < nums.length; j++){
            let t = countArray(nums,i,j);
            count = Math.max(count,t);
        }
    }
    return count;
};
var countArray = function(nums,x,y) {
    let ct = 0;
    for (let i = x; i <= y; i++) {
        ct = ct + nums[i];
    }
